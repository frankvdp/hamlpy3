...
def watched_extension(extension):
    ...
    for ext in constants.HAMLPY_VALID_EXTENSIONS:
...
def watch_folder():
    """
    Main entry point. Expects one or two arguments
    (the watch folder + optional destination folder).
    """
    ...
    if argv_len in (2, 3):
        ...
        destination = os.path.realpath(
            argv_len == 3 and os.path.realpath(sys.argv[2]) or folder)

        print("Watching %s at refresh interval %s seconds" %
              (folder, CHECK_INTERVAL))
...
def _watch_folder(folder, destination):
    """
    Compares "modified" timestamps against the "compiled" dict,
    calls compiler if necessary.
    """
    ...
    for dirpath, dirnames, filenames in os.walk(folder):
        ...
        for filename in filenames:
            ...
            if watched_extension(filename) and not filename.startswith('.#'):
                ...
                if (fullpath not in compiled or
...
def compile_file(fullpath, outfile_name):
    ...
    print('%s %s -> %s' %
          (strftime("%H:%M:%S", gmtime()), fullpath, outfile_name))
    ...
    try:
        ...
        haml_lines = codecs.open(
            fullpath, 'r', encoding='utf-8').read().splitlines()
